...
public class SampleProblems implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<ProofProblemDAO> users = List.of(
                // Pergunta 1
                createNDProofProblem("φ → (φ ∨ ψ)", false),
                createNDProofProblem("(φ ∨ φ) → φ", false),
                createNDProofProblem("(φ ∧ ψ) → φ", false),
                createNDProofProblem("φ → (ψ → φ)", false),
                createNDProofProblem("((φ → ψ) ∧ (ψ → γ)) → (φ → γ)", false),
                createNDProofProblem("(φ → (ψ → γ)) → ((φ → ψ) → (φ → γ))", false),
                createNDProofProblem("(φ → ψ) → (φ → (ψ ∨ γ))", false),
                createNDProofProblem("(ψ → γ) → ((φ ∧ ψ) → γ)", false),
                createNDProofProblem("¬(φ ∨ ψ) → ¬φ", false),
                createNDProofProblem("(ψ → γ) → ((φ ∧ ψ) → (φ ∧ γ))", false),

                // Pergunta 2
                createNDProofProblem("¬φ → (φ → ψ)", false),
                createNDProofProblem("((φ → ψ) ∧ ¬ψ) → ¬φ", false),
                createNDProofProblem("(φ → ¬¬φ) ∧ (¬¬φ → φ)", false),
                createNDProofProblem("((φ → ψ) → (¬ψ → ¬φ)) ∧ ((¬ψ → ¬φ) → (φ → ψ))", false),
                createNDProofProblem("⊥ → φ", false),
                //createNDProofProblem("⊤", false),
                createNDProofProblem("φ ∨ ¬φ", false),
                createNDProofProblem("((φ → δ) → φ) → φ", false),
                createNDProofProblem("φ ∨ (φ → ψ)", false),
                createNDProofProblem("(φ → ψ) ∨ (ψ → δ)", false),

                // Pergunta 3
                createNDProofProblem("¬φ ∨ ψ; φ → ψ", false),
                createNDProofProblem("φ → ψ; ¬φ ∨ ψ", false),
                createNDProofProblem("¬(φ ∧ ψ); ¬φ ∨ ¬ψ", false),
                createNDProofProblem("¬φ ∨ ¬ψ; ¬(φ ∧ ψ)", false),
                createNDProofProblem("¬(φ ∨ ψ); ¬φ ∧ ¬ψ", false),
                createNDProofProblem("¬φ ∧ ¬ψ; ¬(φ ∨ ψ)", false),
                createNDProofProblem("φ ∨ (ψ ∧ δ); (φ ∨ ψ) ∧ (φ ∨ δ)", false),
                createNDProofProblem("(φ ∨ ψ) ∧ (φ ∨ δ); φ ∨ (ψ ∧ δ)", false),
                createNDProofProblem("φ ∧ (ψ ∨ δ); (φ ∧ ψ) ∨ (φ ∧ δ)", false),
                createNDProofProblem("(φ ∧ ψ) ∨ (φ ∧ δ); φ ∧ (ψ ∨ δ)", false),
                createNDProofProblem("(φ → ψ) ∧ (ψ → φ); ((φ ∧ δ) → (ψ ∧ δ)) ∧ ((ψ ∧ δ) → (φ ∧ δ))", false),
                createNDProofProblem("¬(¬φ ∨ ¬ψ); φ ∧ ψ", false)
                ...
                //createNDProofProblem("a; ¬¬a", false),
                //createNDProofProblem("a → ¬¬a", false),
                //createNDProofProblem("(p → q) → q; ¬q → p", false),
                //createNDProofProblem("p → (q ∨ r); (p → q) ∨ (p → r)", false),
